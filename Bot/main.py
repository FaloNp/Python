...
def Start():  # funkcja przycisku ktora otwiera nowe okno
    if ButtonList:
        ...
        Error(error)
        #error = Label(root, text="Wybierz najpierw jakies przyciski")
        #root.wm_attributes('-transparentcolor', 'black')
        #error.place(relwidth=0.2, relheight=0.2, relx=0.5, rely=0.7)
def Clock():
    i = 5
    while (i > 0):
...
def OpenSetting():  # funkcja przycisku ktora otwiera nowe okno
    ...
    # buttony
    button_add = Button(setting, text="Zapisz", command=File)
    ...
    button_setting = Button(setting, text="Ustawienia", command=OpenSetting)
    button_setting.place(relwidth=0.2, relheight=0.1, relx=0.7, rely=0.5)
...
def SetOption(x, y):
    if len(x) == 1:
        ...
        if y.isdigit():
            PressList.append(y)
            for i, nazwa in enumerate(ButtonList):  # przekazuje tekst z fumckji tk.Entry
        ...
        error="Przycisk powienien byc pojedynczy"
        Error(error)
def Reset():
    del PressList[:]
    del ButtonList[:]
